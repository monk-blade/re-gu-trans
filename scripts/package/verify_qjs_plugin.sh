...
if command -v strings >/dev/null 2>&1; then
  ...
  TEXT="$(LC_ALL=C grep -a -o -E 'writeFileAtomic|getCandidateAt|commitNotifier|prefixSearch|loadBinaryFile|gujaratiModelAvailable|transliterateNBest' "$PLUGIN" || true)"
fi

for symbol in writeFileAtomic getCandidateAt commitNotifier prefixSearch loadBinaryFile gujaratiModelAvailable transliterateNBest; do
