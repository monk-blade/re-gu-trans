...
// Common interface so the loader can pick an architecture at runtime based on
// which model files are present in a pack, without the two very different
// inference strategies (single-pass CTC vs. autoregressive seq2seq) knowing
// about each other.
class NeuralModel {
 public:
  virtual ~NeuralModel() = default;
  virtual std::string nbest(const std::string& roman, int requested) = 0;
  virtual const char* version() const = 0;
};

// ---------------------------------------------------------------------------
// gu-transformer-ctc-v3: single-pass CTC model (gujarati_xlit.int8.onnx +
// vocab.tsv). One ONNX Runtime call per query; ~2-5ms warm.
// ---------------------------------------------------------------------------
struct PrefixScore {
...
bool ctcAssetsExist(const std::string& root) {
  return fileExists(root + "/gujarati_xlit.int8.onnx") && fileExists(root + "/vocab.tsv");
}

class CtcModel : public NeuralModel {
 public:
  explicit CtcModel(const std::string& root)
      : env_(ORT_LOGGING_LEVEL_WARNING, "akshar-gu-ctc"),
        options_(),
        session_(nullptr) {
  ...
  const char* version() const override { return modelVersion_.c_str(); }

  std::string nbest(const std::string& roman, int requested) override {
...
// ---------------------------------------------------------------------------
// indicxlit-fairseq-v1.0: seq2seq (encoder+decoder, autoregressive beam
// search) inference for the AI4Bharat IndicXlit checkpoint, exported to ONNX
// by scripts/export/export_indicxlit_onnx.py. Every candidate requires up to
// kMaxDecodeSteps sequential decoder calls, unlike the single-pass CTC model.
// ---------------------------------------------------------------------------
...
bool indicxlitAssetsExist(const std::string& root) {
  return fileExists(root + "/indicxlit_encoder.onnx") &&
         fileExists(root + "/indicxlit_decoder_v2.onnx") &&
         fileExists(root + "/vocab.json");
}

class IndicXlitModel : public NeuralModel {
 public:
  explicit IndicXlitModel(const std::string& root)
      : env_(ORT_LOGGING_LEVEL_WARNING, "akshar-gu-xlit"), options_(), encoder_(nullptr), decoder_(nullptr) {
    options_.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_ALL);
    options_.SetIntraOpNumThreads(1);
 ...
 private:
  ...
  std::string modelVersion_{"indicxlit-fairseq-v1.0"};
};

std::unique_ptr<NeuralModel> loadModel(const std::string& root) {
  if (indicxlitAssetsExist(root)) return std::make_unique<IndicXlitModel>(root);
  if (ctcAssetsExist(root)) return std::make_unique<CtcModel>(root);
  throw std::runtime_error("no recognized model assets under " + root);
}
...
AKSHAR_EXPORT const char* akshar_gu_model_version() {
