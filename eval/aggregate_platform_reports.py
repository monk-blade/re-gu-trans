...
One or more reports may exist per platform now (Linux and macOS each build
multiple arch/model variants), so every report is checked independently and
each required platform must be covered by at least one report.
"""
...
# Keep in sync with the 45MB budget in scripts/package/validate_neural_model_pack.sh
MODEL_PACK_MAX_BYTES = 45 * 1024 * 1024
...
def main() -> int:
    ...
    if not reports:
        raise SystemExit("expected at least one platform report, got none")
    by_platform: dict[str, list[dict]] = {}
    for report in reports:
        by_platform.setdefault(report.get("platform"), []).append(report)
    missing = REQUIRED - set(by_platform)
    if missing:
        raise SystemExit(f"platform reports missing coverage for: {sorted(missing)}")

    failures = []
    variants: dict[str, list[str]] = {}
    for platform, platform_reports in sorted(by_platform.items()):
        variants[platform] = sorted(r.get("arch") for r in platform_reports)
        for report in platform_reports:
            variant = report.get("arch") or "?"
            label = f"{platform}:{variant}"
