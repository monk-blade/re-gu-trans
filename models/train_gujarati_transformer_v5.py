#!/usr/bin/env python3
"""Train Gujarati Transformer-CTC v5: same architecture and CTC-only training
as v4, but distilled from IndicXlit pseudo-labels across the *whole* training
corpus rather than only the 15+ char long tail (see
scripts/generate_indicxlit_pseudolabels_v5.py). The teacher beats the v4
student by ~7pp top1/recall@6 everywhere, not just on long words
(eval/indicxlit_ab_summary.json) -- v5 goes after that gap.

Two things carry over unchanged from v4: the sequence-level (not frame-level)
distillation scheme in load_pseudolabels(), and the plain-CTC-loss-only
training loop (IndicXlit's autoregressive decoder is still architecturally
incompatible with frame-level KD). What's new: --max-train-seconds (a wall-
clock backstop so a 6h GPU budget is respected even if per-epoch time comes
in higher than expected) and a 3-tier oversampling scheme that extends the
v3/v4 short-word oversample idea to the newly-pseudolabel-covered medium
length range, not just the short/long extremes.
"""
...
MODEL_VERSION = "gu-transformer-ctc-v5"
...
def main() -> int:
    ...
    # v4 used 15 (long tail only); v5's generator can label from any floor --
    # keep this in sync with whatever --min-length the pilot in Phase 0/1 settled on.
    parser.add_argument("--long-roman-threshold", type=int, default=4)
...
class BucketedOversampledPairDataset(Dataset):
    """Extends v3/v4's binary short(<=4)/long-pseudolabeled oversample split
    into three tiers, so the "more training signal density" idea (see
    train_gujarati_transformer_v4.load_pseudolabels()'s docstring) also
    covers the medium-length range v5 newly has partial gold-only coverage
    for once --long-roman-threshold is lowered below 15.

    - short (len <= short_max_length): unchanged from v3/v4 -- 2 extra copies.
    - medium (short_max_length < len < long_roman_threshold, gold-only, i.e.
      not in pseudo_romans): `medium_oversample` extra copies.
    - pseudo (roman in pseudo_romans, len > short_max_length): `pseudo_oversample
      - 1` extra copies, same semantics as v4's long_pair_oversample.

    Short and pseudo tiers are kept mutually exclusive (a short word never
    gets pseudo-tier weight on top of its short-tier weight): with v5's
    --long-roman-threshold lowered enough that most of the corpus is
    "pseudo-labeled", a short word that's also pseudo-labeled would otherwise
    stack both tiers' extra copies, ending up ~2x more oversampled than any
    short word ever was in v3/v4. A first full-corpus run with that stacking
    left in regressed two previously-passing reference words (ko, wah) even
    though aggregate accuracy improved -- this exclusion is the fix, keeping
    short words at exactly v3/v4's proven oversample weight.
    """
    ...
    def __init__(
        ...
        augment,
        seed,
        pseudo_romans: set[str],
        short_max_length: int,
        long_roman_threshold: int,
        pseudo_oversample: int,
        medium_oversample: int,
    ):
        base = list(pairs)
        short = [pair for pair in base if len(pair[0]) <= short_max_length]
        medium = [
            pair
            for pair in base
            if short_max_length < len(pair[0]) < long_roman_threshold and pair[0] not in pseudo_romans
        ]
        pseudo = [
            pair
            for pair in base
            if pair[0] in pseudo_romans and len(pair[0]) > short_max_length
        ]
        self.pairs = (
            base
            + short + short
            + medium * max(0, medium_oversample)
            + pseudo * max(0, pseudo_oversample - 1)
        )
